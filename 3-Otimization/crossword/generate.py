...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if overlap is not None:
            (a, b) = overlap
            for x_dom in set(self.domains[x]):
                removed = True

                for y_dom in self.domains[y]:
                    if x_dom[a] is not y_dom[b]:
                        removed = False

                if removed:
                    self.domains[x].remove(x_dom)
                    result = True
    ...
    def backtrack(self, assignment):
        ...
        if self.assignment_complete(assignment):
            ...
            unsigned_var = self.select_unassigned_variable(assignment)
            for domain in self.order_domain_values(unsigned_var, assignment):
                assignment_copy = copy.deepcopy(assignment)
                assignment_copy[unsigned_var] = domain

                if self.consistent(assignment_copy):
                    assignment[unsigned_var] = domain
                    result = self.backtrack(assignment)
                    if result is not None:
                        return result
                assignment.pop(unsigned_var, None)
            return None
